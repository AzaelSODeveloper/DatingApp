...
@Component({
  ...
  imports: [],
  templateUrl: './user-mamangement.html',
  styleUrl: './user-mamangement.css',
})
export class UserMamangement implements OnInit {
  ...
  updateRoles(){
    if(!this.selectedUser) return;
    this.adminService.updateUserRoles(this.selectedUser.id.toString(), this.selectedUser.roles).subscribe({
      next: updatedRoles => {
      ...
      error: error => console.log('failed updating roles', error)
    })
  }
}
